...
public class FullTextIndex {
    ...
    public Stream<SearchHit> search(@Name("label") String label, @Name("query") String query) {
        ...
        return db.index().forNodes(index).query(query).stream().map(SearchHit::new);
    }
    ...
    /**
    * This is the third procedure defined in this class, it is used to update the
    * index with nodes that should be queryable. You can send the same node multiple
    * times, if it already exists in the index the index will be updated to match
    * the current state of the node.
    *
    ...
    */
    ...
    public void ModifyProbability(@Name("nodeId") long nodeId) {
        ...
        node.setProperty("probability", 0);
